...
import { apiAdminGetStats } from "@/lib/api";
...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    fetchMarkets();
    apiAdminGetStats().then(res => { if (res.ok && res.data) setStats(res.data); });
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  const statCards = [
    { label: "Total Markets",   value: stats?.totalMarkets ?? "—",        color: "#6366f1", bg: "rgba(99,102,241,0.1)",   icon: <BarChart3 size={22} /> },
    { label: "Open Markets",    value: stats?.openMarkets ?? "—",         color: "#10b981", bg: "rgba(16,185,129,0.1)",   icon: <Activity size={22} /> },
    { label: "Settled",         value: stats?.settledMarkets ?? "—",      color: "#8b8fa8", bg: "rgba(139,143,168,0.1)",  icon: <CheckCircle2 size={22} /> },
    { label: "Total Trades",    value: stats?.totalTrades ?? "—",         color: "#f59e0b", bg: "rgba(245,158,11,0.1)",   icon: <TrendingUp size={22} /> },
    { label: "Active Trades",   value: stats?.activeTrades ?? "—",        color: "#ec4899", bg: "rgba(236,72,153,0.1)",   icon: <Clock4 size={22} /> },
    { label: "Total Volume",    value: stats ? fmt(stats.totalVolume, true) : "—", color: "#10b981", bg: "rgba(16,185,129,0.1)", icon: <DollarSign size={22} /> },
  ];
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: 20 }}>
        ...
        <div className="card" style={{ padding: 20 }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
            ...
            <Link href="/manage" style={{ fontSize: 12, color: "var(--emerald)", textDecoration: "none", display: "flex", alignItems: "center", gap: 3 }}>
        ...
        <div className="card" style={{ padding: 20 }}>
          ...
          {[
            ...
            { label: "Active Traders", value: stats?.activeTraders ?? "—",  color: "#10b981" },
            { label: "Total Trades",   value: stats?.totalTrades ?? "—",    color: "#f59e0b" },
            { label: "Active Trades",  value: stats?.activeTrades ?? "—",   color: "#ec4899" },
